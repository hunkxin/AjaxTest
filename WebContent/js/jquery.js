...
//		});
		...
		$("#show").on('click',".single",function(){
			...
			var right = $(this).parent().parent().width();
			right = left+right+1;
			var args = {"time": new Date(),"id":$(this).html()};
			
			$.post(url,args,function(data){
				getcontent(data,top,right);
				//getalluser(data);
			},"json")
			return false;
		});
	...
	function getcontent(data,top,right){
		//<table border="1" cellpadding="2" cellspacing="1" style="margin: 0 auto">
		$("#showsingle").empty();
		$("#showsingle").attr("style","background-color:#FFFF77;position:absolute;top:"+top+"px;left:"+right+"px;");
		$("#showsingle").append("<table id='showsingletable' border='1' cellpadding='2' cellspacing='1' style='margin: 0 auto'></table>");
